<?php
/**
 * Real Email Contact Form Handler
 * Versione con invio email reale tramite SMTP
 */
...
try {
    ...
    // Salva email per test
    $email_file = 'data/last_email.html';
    file_put_contents($email_file, $email_body);
    
    // Prova invio email reale
    $email_sent = false;
    $email_error = '';
    
    try {
        // Configurazione SMTP per mail() nativo
        ini_set('SMTP', 'smtp.gmail.com');
        ini_set('smtp_port', '587');
        ini_set('sendmail_from', 'noreply@federcomtur.it');
        ...
        // Prova invio
        $email_sent = mail('denislavgallo2005@gmail.com', $subject, $email_body, implode("\r\n", $headers));
        
        if (!$email_sent) {
            $email_error = 'Invio email fallito (controlla configurazione SMTP)';
        }
        ...
        $email_error = 'Errore invio email: ' . $e->getMessage();
    }
    ...
    if ($email_sent) {
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Richiesta salvata. Email non inviata: ' . $email_error . '. Controlla data/last_email.html per vedere il contenuto.'
        ]);
    }
